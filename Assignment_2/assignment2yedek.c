...
int main (int argc, char *argv[])
{
	...
	int currChild = -1;
	int childList[3]; /*child process list*/
	int i = 0 ; /*order of the running child process*/
	...
	else
	{
		...
		if (currChild == childList[0])
		{
#if DEBUG
			printf("child 1 is starting....\n");
#endif
			/*returning the semaphore ids for KEYSEM1 and KEYSEM */
			...
			for (i = 1; i <= 1001; i++)
			{
				...
				{
#if DEBUG
					printf("Child 1 write %d to shared memory 1\n", i);
#endif
					*globalcp1 = i;
				}
				...
				sem_wait(lockP1, 1);
#if DEBUG			
				printf("Child 1 continues again\n");
#endif
			}
		...
		else if (currChild == childList[1])
		{
#if DEBUG
			printf("child 2 is starting....\n");
#endif
			...
			{
#if DEBUG
				printf("Child 2 waits signal from child process 1\n");
#endif
				/* waiting for the semaphore with id=lockP2 to enter the critical section */
				sem_wait(lockP2, 1);
#if DEBUG
				printf("Child 2 continues\n");
#endif
				localInt = *globalcp1;
				printf("localInt %d\n",localInt);
				if (localInt == 0)
				...
				if (localInt % 3 != 0)
				{
					*globalcp2 = localInt;
					...
					/* waiting for a second */
					//sleep(2);
					//printf("globalcp2 = %d\t globalcp1 = %d\n", *globalcp2, *globalcp1);	
		...
		else if (currChild == childList[2])
		{
#if DEBUG
			printf("child 3 is starting....\n");
#endif
			...
			{
#if DEBUG
				printf("Child 3 waits signal from child process 2\n");
#endif
				/* waiting for the semaphore with id=lock to enter the critical section */
				sem_wait(lockP3, 1);
#if DEBUG
				printf("Child 3 continues\n");
#endif
