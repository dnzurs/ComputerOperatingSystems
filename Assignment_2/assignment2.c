...
#define KEYSEM  	ftok(strcat(get_current_dir_name(), argv[0]+1), 1)	// for waiting of main process
#define KEYSEM1 	ftok(strcat(get_current_dir_name(), argv[0]+1), 2)	// for waiting of child process 1
#define KEYSEM2  	ftok(strcat(get_current_dir_name(), argv[0]+1), 3)	// for waiting of child process 2
#define KEYSEM3 	ftok(strcat(get_current_dir_name(), argv[0]+1), 4)	// for waiting of child process 3
#define KEYSHM1  	ftok(strcat(get_current_dir_name(), argv[0]+1), 5)	// for shared memory for genarting number
#define KEYSHM2  	ftok(strcat(get_current_dir_name(), argv[0]+1), 6)	// for shared memory for numbers that can be divided by 2
	...
	else
	{
		...
		else if (childList[1] == getpid())
		{
			while (1)
			{
				...
				/* updating the value of the shared memory segment */
				*globalcp2 = (localInt % 2 != 0) ? localInt : -1;
